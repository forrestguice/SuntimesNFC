...
import static com.forrestguice.suntimes.nfc.espresso.matcher.ViewMatchersContrib.hasDrawable;
import static com.forrestguice.suntimes.nfc.espresso.matcher.ViewMatchersContrib.navigationButton;
...
public abstract class TestRobot<T>
{
    ...
    public static abstract class ActivityRobot<T> extends TestRobot<T>
    {
        ...
        public T assertActionBar_homeButtonShown(boolean shown) {
            onView(allOf(navigationButton(), hasDrawable(R.drawable.ic_launcher_action))).check(shown ? assertShown : doesNotExist());
            return robot;
        }
    ...
    public static void setAnimationsEnabled(boolean enabled) throws IOException
    {
        if (Build.VERSION.SDK_INT >= 21)
        {
            ...
            Log.w("TEST", "setAnimationsEnabled requires api21+");
        }
    }
